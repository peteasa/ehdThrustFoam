...
electrode_position=-0.02
electrode_radius=0.01
far_field_mesh_size=0.002
thrust_topsurface_mesh_size=0.0005
thrust_surface_mesh_size=0.0015
electrode_mesh_size=0.0004
run(`./set_config_property.sh name unstructured_gmsh_$(thrust_surface_mesh_size)_$(far_field_mesh_size)`)
touch("ehdthrust.geo")
open("ehdthrust.geo","w")do io
    ...
    println(io,"Point($(n_thrust_surface_points+6)) =  {$(electrode_position - electrode_radius), 0.0, 0.0, $electrode_mesh_size};")
    println(io,"Point($(n_thrust_surface_points+7)) =  {$electrode_position, +$(electrode_radius), 0.0, $electrode_mesh_size};")
    println(io,"Point($(n_thrust_surface_points+8)) =  {$(electrode_position + electrode_radius), 0.0, 0.0, $electrode_mesh_size};")
    println(io,"Point($(n_thrust_surface_points+9)) =  {$electrode_position, -$(electrode_radius), 0.0, $electrode_mesh_size};")
    println(io,"Point($(n_thrust_surface_points+10)) =  {$electrode_position, 0.0, 0.0, $electrode_mesh_size};")
    ...
    # in practice paraFoam shows the normals point in the opposite direction
    upper = wall_upper + 1
    println(io,"Spline($l) = {1:$(n_points_per_side+1)};") ; l += 1
    lower = upper + 1
    println(io,"Spline($l) = {$(n_points_per_side+1):$n_thrust_surface_points, 1};") ; l += 1
    ...
    element = c
    pelem = lower + 1 # first of four circle segments
    ...
    """)
    println(io,"""Physical Surface("NELEMENT")={surfaceVector[$upper], surfaceVector[$lower]};""")
